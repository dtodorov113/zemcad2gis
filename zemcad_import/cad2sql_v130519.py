...
class czLayer():
    ...
    def __SQLDictTexts(self, _table):
        '''
        a text seems to mean a kind of point with some data attached
        '''
        PERMITTED_DIFFERENCE = 3 #2 #zaradi dobaveni poleta geom i import_id
        ...
        for item in self.Texts:
            ...
            razlika = (len(arrKeys) - len(arrValues)) - PERMITTED_DIFFERENCE
            #makes sure the difference between the keys and values is 0
            if razlika > 0:
            ...
            #once razlika it's zero continues
            if len(arrValues) == len(arrKeys) - PERMITTED_DIFFERENCE:

                x = self.Parent.RefX + float(arrValues[X_INDEX])
                y = self.Parent.RefY + float(arrValues[Y_INDEX])
    ...
    def __SQLDictPoints(self, _table):
        ...
        for item in self.Points:
            ...
            for i in range(len(arrKeys) - len(
                    arrValues)):  # dopylwam s prazni stojnosti ako kluchowete sa poweche ot stojnostite
    ...
    def __SQLDictSymbols(self, _table):
        '''
        symbol seems to be a kind of point
        '''
        arrKeys = self.__SQLColumns(_table)
        arrValues = []
        listTmp = []
    ...
    def __SQLDictLines(self, _table):
        ...
        arr_v = []
        iLine = 0
        ZEM_INCORECCT_VALUE_COUNT = 6
        CAD_INCORRECT_VALUE_COUNT = 4
        LNEID_INDEX = 2

        for item in self.Lines:
            p = re.search('[^\x20-\x7E]', item)
            if not p:
                arrValues = item.replace("; ;", ";").strip(";").strip().split(";")
                arrValues2 = arrValues[0].strip().split(" ")
                if self.Parent.IsZEM == False:
                    if len(arrValues2) == ZEM_INCORECCT_VALUE_COUNT:
                        arrValues2.append('0')
                elif len(arrValues2) == CAD_INCORRECT_VALUE_COUNT:
                    arrValues2.append('0')
                arrValues.remove(arrValues[0]);
                arrTmpYX = []
                ProblemInLine = False
                for el in arrValues:
                    arrValues3 = el.strip().split(" ")
                    if len(arrValues3) >= 2:
                        try:
                            ...
                            if self.Parent.FillLinePointTable == True:
                                ...
                                # print tmpSQl
                                # tmpSQl = """INSERT INTO cz_service_line_point ("""+"import_id, layer_name, line_id, point_xy"+""") VALUES("""+ "%s, '"%self.Parent.ImportID + self.Name + "', %s, %s" %(polyIDforServiceTable ,lineID) +""")"""
                                tmpSQl = """INSERT INTO cz_service_line_point (""" + "import_id, layer_name, line_id, point_xy" + """) VALUES(""" + "%s, '" % self.Parent.ImportID + self.Name + "', %s, '" % (
                                    LneID) + yx + """')"""
                                c.execute(tmpSQl)
                                c.close()
    ...
    def __SQLDictContures(self, _table):
       ...
       # countTexts = 0
        for item in self.Contures:
            ...
            polyIDforServiceTable = arrValues2[ID_INDEX]
                ...
                if len(arrLinesIDs) > 0:
                    ...
                    for lineID in arrLinesIDs:
                        ...
                        if self.Parent.FillContureLineTable:
                            ...
                            # tmpSQl = """INSERT INTO cz_service_conture_line ("""+"import_id, layer_name, conture_id, line_id"+""") VALUES("""+ "%s, '"%self.Parent.ImportID + self.Name + "', %s, %s" %(polyIDforServiceTable ,lineID) +""")"""
                            tmpSQl = """INSERT INTO cz_service_conture_line (""" + "import_id, layer_name, conture_id, line_id" + """) VALUES(""" + "%s, '" % self.Parent.ImportID + self.Name + "', '%s', '%s'" % (
                                polyIDforServiceTable, lineID) + """)"""
                            c.execute(tmpSQl)
                            c.close()
                            self.Parent.db.commit()
                        # ======================================================================
                                    ...
                                    if lF == cL:
                                        ...
                                        if len(arrPolyCoords) == 0:
                                            ...
                                            if cF == cL:
                                                ...
                                                c = self.Parent.db.cursor()
                                                tmpSQl = """INSERT INTO cz_error (layer_name, object_type, objet_id, note) VALUES('""" + self.Name + """', '""" + """conture""" + """', '""" + str(
                                                    polyIDforServiceTable) + """', '""" + """skipped conture""" + """')"""
                                                # print tmpSQl
                                                c.execute(tmpSQl)
    ...
    def __SQLDictConturesInnerPoints(self, _table):
        '''
        a point in each polygon
        '''
        arrKeys = self.__SQLColumns(_table)
        listTmp = []

        for item in self.Contures:
            arrValues = item.strip().split(";")
            arrValues2 = arrValues[0].strip().split(" ")

            isCoordsOK = True
            try:
                x = self.Parent.RefX + float(arrValues2[X_INDEX])
                y = self.Parent.RefY + float(arrValues2[Y_INDEX])
            except:
                isCoordsOK = False
            ...
            if isCoordsOK:
                ...
                for i in range(len(arrKeys) - len(
                        arrValues2)):  # dopylwam s prazni stojnosti ako kluchowete sa poweche ot stojnostite
    ...
    def __ListToDict(self, _table):
        '''
        reads data from czLayer and puts it in dictTmp
        '''
        dictTmp = {}
        if _table == "_p":
    ...
    def ImportFeaturesToBD(self, _table):
        '''
            imports a particular type of feature to the database
            _table is the type of feature to import
            "_p" -> points
            "_s" -> symbols
            "_t" -> texts
            "_l" -> lines
            "_c" -> countries (polygons)
            "_d" -> a point for each country
        '''
        try:
            ...
            if len(dictForSQL) > 0:
                ...
                c.executemany(
                    """INSERT INTO """ + tableName + """(""" + strKeys + """) VALUES(""" + strValues + """)""",
                    dictForSQL)
...
class czFileHandler():
    arr1970 = [
        {'zone': 'k3',
         'proj4': '+proj=lcc +lat_1=43.45678000 +lat_2=43.45680000 +lat_0=3.86550343 +lon_0=23.19729755 +x_0=8496745.16350000 +y_0=0.0 +ellps=krass +towgs84=24,-123,-94,0.02,-0.25,-0.13,1.1 +units=m +no_defs'},
        {'zone': 'k5',
         'proj4': '+proj=lcc +lat_1=42.47925500 +lat_2=42.47927500 +lat_0=3.51876580 +lon_0=26.38994424 +x_0=9497002.61430000 +y_0=0.0 +ellps=krass +towgs84=24,-123,-94,0.02,-0.25,-0.13,1.1 +units=m +no_defs'},
        {'zone': 'k7',
         'proj4': '+proj=lcc +lat_1=43.56352000 +lat_2=43.56354000 +lat_0=3.97536005 +lon_0=26.23175436 +x_0=9503619.34450000 +y_0=0.0 +ellps=krass +towgs84=24,-123,-94,0.02,-0.25,-0.13,1.1 +units=m +no_defs'},
        {'zone': 'k9',
         'proj4': '+proj=lcc +lat_1=42.29330889 +lat_2=42.29332889 +lat_0=3.92530164 +lon_0=23.41990955 +x_0=8506383.44450000 +y_0=0.0 +ellps=krass +towgs84=24,-123,-94,0.02,-0.25,-0.13,1.1 +units=m +no_defs'},
    ]
    tableColumnsService = {
        # "cz_import"   : ("source", "checksum", "import_time", "ekt"),
        "cz_import": (
            "cz_source", "cz_checksum", "cz_import_time", "cz_format", "cz_prefix", "cz_COMMENT", "cz_CONTENTS",
            "cz_COORDTYPE", "cz_DATE", "cz_FIRM", "cz_NAME", "cz_PLANDATA", "cz_PLANNOMER", "cz_PLANSIGN", "cz_PROGRAM",
            "cz_REFERENCE", "cz_SCALE", "cz_VERSION", "cz_WINDOW", "cz_EKATTE", "cz_EKNM", "cz_PLANNUMBER"),
    ...
    tableColumnsCAD = {
        ...
        "_p": (
            "import_id", "layer_name", "pstlc", "t", "n", "x", "y", "h", "k", "mx", "my", "kh", "mh", "mst", "msg",
            "sgn",
            "cen", "ono", "b", "d", "geom"),
        "_s": ("import_id", "layer_name", "pstlc", "t", "n", "x", "y", "a", "m", "b", "d", "geom"),
        "_t": (
            "import_id", "layer_name", "pstlc", "t", "n", "x", "y", "h", "b", "d", "r", "j", "ps", "tnp", "ss", "geom"),
    ...
    def __AddToImportTable(self):
        '''adds record of the import in the appropriate table'''
        c = self.db.cursor()
        dictTmp = self.Header
    ...
    def __DetermineFormatPrefixByLineAttributeCount(self, _fileAsString):
        ...
        if c in ZEM_ATTR_COUNTS:
            return "z_"
        elif c in CAD_ATTR_COUNTS:
            return "c_"
    ...
    def Sumarize(self, _resultTable, _sourceTables):
        ...
        if len(arrTables) > 0:
            ...
            for t in arrTables:
                plpy.execute(
                    "SELECT a.attname FROM pg_catalog.pg_attribute a WHERE a.attnum > 0 AND NOT a.attisdropped AND a.attrelid = (SELECT c.oid FROM pg_catalog.pg_class c LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace WHERE c.relname ~ '^(" +
                    t[0] + ")$' AND pg_catalog.pg_table_is_visible(c.oid))");
            ...
            if len(plpy.fetchall()) == 0:
                if not _sourceTables.endswith("_tbl"):
                    ...
                    plpy.execute(
                        "CREATE INDEX " + _resultTable + "_geom_gist ON " + _resultTable + " USING gist (geom)")
                else:
                    plpy.execute("CREATE TABLE " + _resultTable + "( " + strKeysNew +
                                 ", CONSTRAINT " + _resultTable + "_pkey PRIMARY KEY (gid) " +
                                 ")")
                ...
                plpy.execute(
                    "SELECT a.attname FROM pg_catalog.pg_attribute a WHERE a.attnum > 0 AND NOT a.attisdropped AND a.attrelid = (SELECT c.oid FROM pg_catalog.pg_class c LEFT JOIN pg_catalog.pg_namespace n ON n.oid = c.relnamespace WHERE c.relname ~ '^(" + _resultTable + ")$' AND pg_catalog.pg_table_is_visible(c.oid))");
    ...
    def PrepareFile(self):
        '''
        creates an intermediate file to be processed
        '''
        fi = open(self.czFullFileName, 'r')
        fo = open(self.preparedFileName, 'w')
        ...
        strTmp = re.compile('  +').sub(' ', strTmp)
        strTmp = re.compile('\r') .sub('', strTmp)  # za npwite redowe pod linux
        ...
        strTmp = re.compile('\n\"').sub(';\"', strTmp)
        strTmp = re.compile(';LAYER ').sub('\nLAYER ', strTmp)
        strTmp = re.compile(';CONTROL ').sub('\nCONTROL ', strTmp)
        strTmp = re.compile(';ET ').sub('\nET ', strTmp)
        strTmp = re.compile('END_LAYER;').sub('END_LAYER\n;', strTmp)
        strTmp = re.compile(';TABLE ').sub('\nTABLE ', strTmp)
        ...
        # [[x for x in line.split()] for line in open(self.preparedFileName)]
        # [self.PreparedLines.append(line) for line in open(self.preparedFileName)]
        self.MD5 = hashlib.md5(open(self.preparedFileName).read()).hexdigest()
        self.KoiToWin()

        [self.PreparedLines.append(line) for line in open(self.preparedFileName)]
    def KoiToWin(self):
        dstsrch = [chr(191), chr(192), chr(193), chr(194), chr(195), chr(196), chr(197), chr(198), chr(199), chr(200),
                   chr(201), chr(202), chr(203), chr(204), chr(205), chr(206), chr(207), chr(208), chr(209), chr(210),
                   chr(211), chr(212), chr(213), chr(214), chr(215), chr(216), chr(217), chr(218), chr(219), chr(220),
                   chr(221), chr(222), chr(223), chr(224), chr(225), chr(226), chr(227), chr(228), chr(229), chr(230),
                   chr(231), chr(232), chr(233), chr(234), chr(235), chr(236), chr(237), chr(238), chr(239), chr(240),
                   chr(241), chr(242), chr(243), chr(244), chr(245), chr(246), chr(247), chr(248), chr(249), chr(250),
                   chr(251), chr(252), chr(253), chr(254), chr(255)]
        srcsrch = [chr(127), chr(128), chr(129), chr(130), chr(131), chr(132), chr(133), chr(134), chr(135), chr(136),
                   chr(137), chr(138), chr(139), chr(140), chr(141), chr(142), chr(143), chr(144), chr(145), chr(146),
                   chr(147), chr(148), chr(149), chr(150), chr(151), chr(152), chr(153), chr(154), chr(155), chr(156),
                   chr(157), chr(158), chr(159), chr(160), chr(161), chr(162), chr(163), chr(164), chr(165), chr(166),
                   chr(167), chr(168), chr(169), chr(170), chr(171), chr(172), chr(173), chr(174), chr(175), chr(176),
                   chr(177), chr(178), chr(179), chr(180), chr(181), chr(182), chr(183), chr(184), chr(185), chr(186),
    ...
    def FillLayersAndTablesFromPreparedFile(self):
        '''
        creates czLayer()'s for each layer in the file
        records data from tables
        '''
        # for tables
        strTableDictName = ""
        arrTableRecords = []
        ...
        for line in open(self.preparedFileName):
            ...
            if line.startswith("END_HEADER"):
                ...
                if self.Header.has_key("VERSION"):
                    self.Version = self.Header["VERSION"]
                if self.Header.has_key("EKATTE"):
                    self.EKATTE = self.Header["EKATTE"]
                if self.Header.has_key("SCALE"):
                    self.Scale = self.Header["SCALE"]
                if self.Header.has_key("CONTENTS"):
                    self.Contents = self.Header["CONTENTS"]
                ...
                if self.FileFormat == "CAD" and self.FormatPrefix == "z_":
                    self.FormatPrefix = "cZ_" + self.Version.replace(".", "_") + "_"
                else:
                    self.FormatPrefix += self.Version.replace(".", "_") + "_"

                for col in self.tableColumnsService["cz_import"]:
                    if (not self.Header.has_key(col.replace("cz_", ""))) and\
                            (col != "cz_source" and col != "cz_checksum" and col != "cz_import_time"
                             and col != "cz_format" and col != "cz_prefix"):
            ...
            # obrabotwam red za layer
                ...
                if ItemsCount <= MIN_ITEMS:  #dyljina na opicanie na teksta T(sledwa red T )
    ...
    def createAreaTable(self):
        ...
        c.execute("set client_encoding to 'WIN'")
        c.executemany("""INSERT INTO """ + tableName + """(""" + strKeys + """) VALUES(""" + strValues + """)""",
                      self.Area)
    ...
    def TableToPostGIS(self, _tableName):
        ...
        c.executemany("""INSERT INTO """ + tableNameForSQL + """(""" + strKeys + """) VALUES(""" + strValues + """)""",
                      dictTmp)
    ...
    def listToDictForTexts(self, _list, _arrKeys):
        ...
        for item in _list:
            ...
            if len(arrValues) <= len(arrKeys) - PERMITTED_DIFF:
                razlika = (len(arrKeys) - len(arrValues)) - PERMITTED_DIFF
                for i in range(razlika):
                    # insyrtwane na tretposlednomqsto
                    arrValues.insert(len(arrValues) - 1, "")

                x = self.RefX + float(arrValues[X_INDEX])
                y = self.RefY + float(arrValues[Y_INDEX])
                arrValues.append("POINT(" + str(y) + " " + str(x) + ")")
        ...
        for item in _list:
            ...
            if len(arrKeys) == len(arrValues):
                ...
                if razlika > 0:
                    ...
                    for i in range(
                            abs(razlika)):  # dopylwam s prazni stojnosti ako kluchowete sa poweche ot stojnostite
